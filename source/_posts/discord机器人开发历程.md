...
现在回头看，这个项目真正有价值的地方，不是“功能很多”，而是终于把边界划清楚了：该认真做事的时候有 `shen-test`，该放松玩的时候有 `shen-play`。这篇文章也按**现在真实在跑的版本**重写一遍，把开发过程、当前功能和一些踩坑后的取舍说清楚。
...
1. 功能越来越多，但边界越来越乱
2. 自然语言触发容易互相打架
3. 一个机器人同时承担“助理”和“玩伴”两种角色，体验会变得拧巴

后来我花了不少时间重构，才把这套东西变成现在比较舒服的样子。

![Discord 机器人开发时间线](/images/260507-1.png)
...
![当前双机器人结构图](/images/260507-2.png)

- `shen-test` 可以安心往效率工具和长期记忆方向做
- `shen-play` 可以放心往娱乐体验和玩法方向做
- 文档、配置、命令边界都会更清楚
...
1. **一次性提醒**：比如明天下午三点提醒我交东西
2. **每日定时任务**：比如每天早上固定发一份简报
...
它会从 `persona/` 文件夹里读取当前可用人设，你可以直接切换。现在这边是**多 persona** 结构，不像 `shen-test` 那样固定只有一套主人设。
...
- 任务系统怎么避免误触发
- 画像和长期记忆怎么配合
- `shen-test` 和 `shen-play` 怎么划清边界
- 抽卡卡片怎么做得既稳定又好看

这些事情每一项都不算“炫技”，但它们决定了项目到底能不能长期用下去。

![功能边界对比图](/images/260507-3.png)
...
- `shen-test`：工作和生活管理主线，偏长期使用
- `shen-play`：娱乐和轻互动主线，偏即时体验
- 两边共享一部分底层配置，但人格和目标不同
- 服务器上作为常驻服务运行
